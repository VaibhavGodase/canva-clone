      ...
      {listOfDesigns.map((design) => (
        <div key={design._id} className="group cursor-pointer">
          <div
            ...
            className="w-[90px] sm:w-[230px] h-[100px] sm:h-[300px] rounded-lg mb-2 overflow-hidden transition-shadow group-hover:shadow-md border border-black "
          >
            {design?.canvasData && (
          ...
          <div className="flex justify-between">
            <p className="font-bold text-[10px] sm:text-sm truncate">{design.name}</p>
            <Trash2
